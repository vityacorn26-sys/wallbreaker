...
console.error('API Error (buyDrawTicket):', e);
return { success: false, error: e?.payload?.error || 'buy_key_failed' };
}
},

async enterDrawTicket() {
try {
return await this.post('/api/draw/ticket/enter');
} catch (e) {
console.error('API Error (enterDrawTicket):', e);
return { success: false, error: e?.payload?.error || 'enter_key_failed' };
}
},
...
return { success: false, keys: 0, entered: 0, max: 2 };
}
}
};
