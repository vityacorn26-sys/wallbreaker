...
async function loadBreachBoard() {
  try {
    ...
    sorted.forEach((player) => {
      ...
      row.innerHTML = `
        <td class="breach-board-nickname">
          <span>${nickname}</span>
          ${entries > 0 ? `<span class="key-badge">+${entries} key${entries>1?'s':''}</span>` : ''}
        </td>
        <td class="breach-board-score">${liveScore.toLocaleString()}</td>
      `;
    ...
    const loaderEl = document.getElementById("breach-board-loader");
    if (loaderEl) {
      loaderEl.innerHTML = `
