...
let drawBuyLocked = false;
let drawEnterLocked = false;
...
const DRAW_STATUS_CHARGING = "PRIZE POOL IS CHARGING IN THE NETWORK";
const DRAW_STATUS_LOCKED = "HACKERS' PRIZE POOL IS LOCKED AND READY TO DROP";
...
function isPrizePoolLocked() {
return Boolean(getConfig()?.DRAW?.LOCKED);
}

function getPrizePoolStatusText() {
return isPrizePoolLocked() ? DRAW_STATUS_LOCKED : DRAW_STATUS_CHARGING;
}
...
buyKey: "BUY KEY",
enterKey: "ENTER KEY",
keyBought: "Zero-Day Key acquired",
keyBuyFail: "Key purchase failed",
keyNoWbc: "Not enough $WBC for Zero-Day Key",
keyEnterOk: "Zero-Day Key entered into current draw",
keyEnterFail: "Key entry failed",
keyNoKeys: "No Zero-Day Keys available",
keyLimitReached: "Maximum 2 keys per draw reached",
keyStatus: (keys, entered, max) => `Keys: ${keys} • Entered: ${entered}/${max}`,
prizeHint: "Buy Zero-Day Keys in Darknet Market, then enter up to 2 keys into the current draw.",
drawBuyBusy: "Key purchase already in progress",
drawEnterBusy: "Key entry already in progress"
},
EN: {
...
...userState,
...data,
balance: Number(data?.balance ?? data?.wbc_balance ?? 0),
...
zeroDayKeys: Number(
data?.zeroDayKeys ??
data?.zero_day_keys_balance ??
data?.zero_day_keys ??
userState.zeroDayKeys ||
0
),
...
function syncDrawState(data) {
drawState.keys = Math.max(0, Number(data?.keys ?? userState.zeroDayKeys ?? 0));
drawState.entered = Math.max(0, Number(data?.entered ?? 0));
drawState.max = Math.max(1, Number(data?.max ?? 2));

userState.zeroDayKeys = drawState.keys;
}

function ensurePrizePoolControls() {
const overlay = document.getElementById("prize-panel-overlay");
if (!overlay) return null;

const panel = overlay.querySelector(".wb-panel");
if (!panel) return null;

let root = document.getElementById("draw-control-card");
if (root) return root;

root = document.createElement("div");
root.id = "draw-control-card";
root.className = "info-card market-info-card draw-control-card";
root.innerHTML = `     <p><strong>ZERO-DAY KEY</strong></p>     <p id="draw-status-line">${DRAW_STATUS_CHARGING}</p>     <p id="draw-key-status-line">Keys: 0 • Entered: 0/2</p>     <p id="draw-hint-line">${t().prizeHint}</p>     <div class="draw-actions-row">       <button id="draw-buy-key-btn" class="wb-button ghost" type="button">${t().buyKey}</button>       <button id="draw-enter-key-btn" class="wb-button premium" type="button">${t().enterKey}</button>     </div>
  `;

const backBtn = panel.querySelector(".wb-back-btn");
if (backBtn && backBtn.nextSibling) {
panel.insertBefore(root, backBtn.nextSibling);
} else {
...
function renderPrizePoolPanel() {
const root = ensurePrizePoolControls();
if (!root) return;

const statusLine = document.getElementById("draw-status-line");
const keyStatusLine = document.getElementById("draw-key-status-line");
const hintLine = document.getElementById("draw-hint-line");
const buyBtn = document.getElementById("draw-buy-key-btn");
const enterBtn = document.getElementById("draw-enter-key-btn");

if (statusLine) statusLine.textContent = getPrizePoolStatusText();
if (keyStatusLine) keyStatusLine.textContent = t().keyStatus(drawState.keys, drawState.entered, drawState.max);
if (hintLine) hintLine.textContent = t().prizeHint;

if (buyBtn) {
buyBtn.textContent = t().buyKey;
buyBtn.disabled = drawBuyLocked;
...
if (enterBtn) {
enterBtn.textContent = t().enterKey;
enterBtn.disabled = drawEnterLocked || drawState.keys <= 0 || drawState.entered >= drawState.max || isPrizePoolLocked();
...
const poolText = getPrizePoolStatusText();
...
if (keysValue) keysValue.textContent = `${Number(drawState.keys || 0).toLocaleString()} • ${drawState.entered}/${drawState.max}`;
...
try {
const result = await API.getDrawStatus();
if (result?.success) {
syncDrawState(result);
updatePrizePoolPanel();
updateAccountPanel();
renderMarketPanel();
}
} catch (e) {
console.error("loadDrawStatus error:", e);
}
}
...
async function buyDrawKey() {
if (drawBuyLocked) {
safeAlert(t().drawBuyBusy);
return;
}
...
try {
const result = await API.buyDrawTicket();

```
if (!result?.success) {
  if (result?.error === "no_wbc") {
    safeAlert(t().keyNoWbc);
  } else {
    safeAlert(t().keyBuyFail);
  }
  return;
}
...
await loadDrawStatus();
updateUI();
safeAlert(t().keyBought);
```

} catch (e) {
console.error("buyDrawKey error:", e);
safeAlert(t().keyBuyFail);
} finally {
drawBuyLocked = false;
renderPrizePoolPanel();
...
async function enterDrawKey() {
if (drawEnterLocked) {
safeAlert(t().drawEnterBusy);
return;
}

drawEnterLocked = true;

try {
const result = await API.enterDrawTicket();

```
if (!result?.success) {
  if (result?.error === "no_keys") {
    safeAlert(t().keyNoKeys);
  } else if (result?.error === "limit") {
    safeAlert(t().keyLimitReached);
  } else {
    safeAlert(t().keyEnterFail);
  }
  return;
}
...
await loadDrawStatus();
updateUI();
safeAlert(t().keyEnterOk);
```

} catch (e) {
console.error("enterDrawKey error:", e);
safeAlert(t().keyEnterFail);
} finally {
drawEnterLocked = false;
renderPrizePoolPanel();
renderMarketPanel();
}
}
...
const infoCard = marketOverlay.querySelector(".market-info-card");
if (infoCard) {
const pEls = infoCard.querySelectorAll("p");
if (pEls[0]) pEls[0].innerHTML = "<strong>Zero-Day Key</strong>";
if (pEls[1]) pEls[1].textContent = `${Number(getZeroDayKeyPrice()).toLocaleString()} ${getCurrency()} = 1 Zero-Day Key`;
if (pEls[2]) pEls[2].textContent = t().zeroDayPersist;

```
let actionWrap = infoCard.querySelector(".zero-day-buy-wrap");
if (!actionWrap) {
  actionWrap = document.createElement("div");
  actionWrap.className = "zero-day-buy-wrap";
  actionWrap.innerHTML = `
    <div class="zero-day-key-visual">ZERO-DAY KEY</div>
    <div class="zero-day-key-meta" id="zero-day-key-meta">${t().keyStatus(drawState.keys, drawState.entered, drawState.max)}</div>
    <button id="zero-day-buy-btn" class="wb-button premium" type="button">${t().buyKey}</button>
  `;
  infoCard.appendChild(actionWrap);
}

const keyMeta = actionWrap.querySelector("#zero-day-key-meta");
const buyBtn = actionWrap.querySelector("#zero-day-buy-btn");

if (keyMeta) keyMeta.textContent = t().keyStatus(drawState.keys, drawState.entered, drawState.max);
if (buyBtn) {
  buyBtn.textContent = t().buyKey;
  buyBtn.disabled = drawBuyLocked;
  buyBtn.onclick = async () => {
...
el.innerHTML = `     <span class="rank-label-main">${rank.name}</span>     <span class="rank-label-meta">${meta}</span>
  `;
}
